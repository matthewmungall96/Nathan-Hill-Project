...
namespace RuntimeScriptField.Inner
{
    ...
    public class ScriptReferenceDrawer : PropertyDrawer
    {
        ...
        {
            if (!monoScriptsCached) CacheMonoScripts();

            if (!scriptreferenceCached) CacheScriptReferenceData(property);
        ...
        private bool IsValidScriptType(Type attemptedType, MonoScript attemptedScript)
        {
            ...
            {
                if (attemptedScript == null) return true;
            ...
            {
                Debug.LogWarningFormat("Cannot assign script {0} (containing the type {1}) to a {2}! The type of the script must derive from {3}!", attemptedScript.name, attemptedType, scriptReference.GetType().Name, requiredReferenceType.Name);
                return false;
            }

            if (script == null) return true;
            if (attemptedType != script.SystemType) return true;
            return false;
        }
        ...
        private static void CacheMonoScripts()
        {
            ...
            foreach (var monoScript in monoScripts)
            {
                ...
                if (type != null) typeToScript[type] = monoScript;
            }
        ...
        private void CacheScriptReferenceData(SerializedProperty property)
        {
            ...
            monoScript = null;
            if (script != null && script.SystemType != null) typeToScript.TryGetValue(script.SystemType, out monoScript);
        ...
        private Type GetGenericScriptRestriction(Type subclassOfScriptReference)
        {
            ...
            while (current != null)
            {
                ...
                if (genericDef == scriptReferenceType) return current.GetGenericArguments()[0];
                current = current.BaseType;
            }
            return null;
        }
